...
df = load_or_create_dataframe('image_descriptions2.csv')
...
def process_image(image_file):
    ...
    for response in generate(model='llava:13b-v1.6', 
                             prompt='describe the image in 30 words with keywords in 10 words strictly. Generate description quickly within 5 minutes at maximum, donot take too much time', 
                             images=[image_bytes], 
                             stream=True):
...
df.to_csv('image_descriptions2.csv', index=False)